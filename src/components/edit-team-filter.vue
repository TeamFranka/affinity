    ...
    async getSettings(){
      ...
      if(teamPriority.settings!==undefined && teamPriority.settings!==null){
        if(teamPriority.settings.feedTabs!==undefined && teamPriority.settings.feedTabs!==null){         
          ...
          newArr.map((x: any)=>{
            const res = this.initalTeam.findIndex((y: any)=>{
              if(y.name==x.name){return y.name == x.name}
              });
            const notFound = this.initalTeam.findIndex((y: any)=>{
            ...
            if(notFound){
              this.initalTeam.splice(notFound,1)
            }
          })
