<template>
    <ion-header>
        ...
        <ion-segment :value="selectedGroup" scrollable>
            <ion-segment-button
                v-for="e in EmojiGroups" :key="e.group"
                :value="e.group"
...
<script>
import {
  IonSearchbar, IonSegment, IonSegmentButton, IonContent, IonHeader,
  IonButton, IonIcon, IonToolbar,
  modalController,
} from '@ionic/vue';
import {
...
export default defineComponent({
  ...
  components: {
    IonSearchbar, IonContent, IonHeader, IonSegment, IonSegmentButton,
    IonButton, IonIcon, IonToolbar,
  },
  data(){
      return {
  ...
  setup(){
      return {
          EmojiAllData, EmojiGroups, closeIcon,
      }
  },
  computed: {
      visibleIcons() {
        if (this.searchValue.length) {
            ...
            EmojiAllData.forEach((g) => {
                g.emojiList.filter((i) =>
                    !!i.tags.find((t) => t.includes(v))
