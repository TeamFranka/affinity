...
describe('Posting to the feed as regular User', () => {
    ...
    it.only('Posts a Link via button', () => {
      ...
      cy.get('[data-cy-type=activity] [data-cy-obj=link] a')
          ...
          .then(($a) => {
            cy.get($a).contains("Everything you ever wanted to know about unfurling but were afraid to ask /or/ How to make your…");
            cy.get($a).parents("[data-cy-obj=link]").contains('Let’s start with the most obvious question first. This is what an “unfurl” is:')
        });
    })
  })
